...
class DQNCritic(BaseCritic):
    ...
    def _build(self, q_func):
        ...
        if self.double_q:
            ...
            update_q_tp1_values = q_func(self.obs_tp1_ph, self.ac_dim, scope="q_func", reuse=True)
            max_action = tf.argmax(update_q_tp1_values, axis=1)
            q_tp1 = tf.reduce_sum(q_tp1_values*tf.one_hot(max_action, self.ac_dim), axis=1)
        else:
